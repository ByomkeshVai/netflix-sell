...
import HotSaleFr from "./HotSaleFr";

const HotSale = () => {
  const [axiosSecure] = useAxiosSecure();
  const { user, loading } = useContext(AuthContext);
  const { refetch, data: hot = [] } = useQuery({
    queryKey: ["hot"],
    enabled: !loading,
    queryFn: async () => {
      const res = await axiosSecure(
        `${import.meta.env.VITE_API_URL}/items/flashSale/hotSale`
      );
        ...
        <div className="max-w-7xl mx-auto">
          <div className="pt-12 ">
            ...
            >
              {hot?.map((hot) => (
                <SwiperSlide>
                  <HotSaleFr hot={hot} refetch={refetch} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
